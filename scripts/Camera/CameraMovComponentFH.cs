...
public class CameraMovComponentFH : MonoBehaviour
{
    ...
    { 
        //A to move left
        if (Input.GetKey(KeyCode.A)) transform.position+= -transform.right * movSpeed;

        //D to move right
        if (Input.GetKey(KeyCode.D)) transform.position += transform.right * movSpeed;
        
        //W to move forward
        if (Input.GetKey(KeyCode.W)) transform.position += transform.forward * movSpeed;
        
        //S to move backwards
        if (Input.GetKey(KeyCode.S)) transform.position += -transform.forward * movSpeed;
    }
    ...
    private void Rotate()
    {
        ...
        if (Input.GetKey(KeyCode.Q)) transform.Rotate(new Vector3(0, -rotSpeed, 0));
        
        //Use E to rotate clock wise
        if (Input.GetKey(KeyCode.E)) transform.Rotate(new Vector3(0, rotSpeed, 0));
    }
