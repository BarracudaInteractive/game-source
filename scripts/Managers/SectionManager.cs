...
public class SelectSection : MonoBehaviour
{
    ...
    public void submit() { startSectionSet(false); moveNodes(); _gameObject.GetComponent<Attrib>().sf = t; }
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (hit.transform)
            {
                ...
                {
                    timeout = -rtime;
                    id = hit.collider.GetComponent<Attrib>().id;
                    rev = hit.collider.GetComponent<Attrib>().reverse;
                    idx = id * 3;
