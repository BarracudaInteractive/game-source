...
public class GameManager : MonoBehaviour
{
    ...
    private void _GameOver(bool D, bool F, bool O, bool S)
    {
        ...
        else if (S) tDefeat.text = "You lost because you fell apart!";
    }
    ...
    private void FixedUpdate()
    {
        ...
        bool stuck = (int) _Controller.GetKPH == 0;
        // add || _isOut when roads are finished
        if (isDamaged || noFuel || stuck)
        {
            _GameOver(isDamaged, noFuel, false, stuck);
            _Controller.StopCar();
        }
    }
    ...
    private void Update()
    {
        if (_hasStarted)
        {
            _fTime += (Time.deltaTime * _fTimeSpeed);
            _SetText();
        }
    }
